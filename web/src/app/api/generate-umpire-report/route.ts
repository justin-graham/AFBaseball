...
const CLOUD_RUN_URL = process.env.UMPIRE_REPORT_SERVICE_URL;

export async function POST(request: Request) {
  try {
    const body = await request.json();
    const { homeTeamName, homeTeamId, awayTeamName, awayTeamId, startDate, endDate, season } = body;

    // Validate inputs
    if (!homeTeamName || !homeTeamId || !awayTeamName || !awayTeamId || !startDate || !endDate) {
      return NextResponse.json(
        { error: 'Missing required fields: homeTeamName, homeTeamId, awayTeamName, awayTeamId, startDate, endDate' },
        { status: 400 }
      );
    }
    ...
    if (CLOUD_RUN_URL) {
      ...
      result = await callCloudRun({ homeTeamName, homeTeamId, awayTeamName, awayTeamId, season: season || 2025, startDate, endDate });
    } else {
      // Local: Execute Python script directly
      result = await callPythonScript({ homeTeamName, homeTeamId, awayTeamName, awayTeamId, season: season || 2025, startDate, endDate });
    }
...
async function callCloudRun(params: any) {
  const { homeTeamName, homeTeamId, awayTeamName, awayTeamId, season, startDate, endDate } = params;
...
async function callPythonScript(params: any) {
  ...
  const scriptPath = path.join(process.cwd(), '../../umpire_report_gen.py');
  ...
  const cmd = `python3 "${scriptPath}" --home-team "${homeTeamName}" --home-team-id "${homeTeamId}" --away-team "${awayTeamName}" --away-team-id "${awayTeamId}" --season ${season} --start-date "${startDate}" --end-date "${endDate}" --output-dir "${process.cwd()}"`;
